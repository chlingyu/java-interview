...
**一句话总结**：ArrayList 初始容量为 **10**，每次扩容为原来的 **1.5 倍**（`oldCapacity + oldCapacity >> 1`），底层调用 `Arrays.copyOf()` 把旧数组元素复制到新数组。
...
> **面试话术**：ArrayList 默认初始容量是 10，空间不足时扩容为原来的 1.5 倍。扩容底层通过 Arrays.copyOf 创建新数组并复制旧数据，所以如果数据量能预估，最好创建时指定容量，避免频繁扩容带来的性能损耗。
