...
### 12. binlog、redo log、undo log 的区别？

> ⭐⭐⭐⭐ 高频 | 难度：⭐⭐⭐

**一句话回答**：redo log 保证事务持久性（崩溃恢复），undo log 保证事务原子性（回滚），binlog 用于主从复制和数据恢复。

**通俗理解**：

把 MySQL 想象成一家银行：

- **redo log（重做日志）** = 柜员的"操作草稿纸"。你来存 1000 块，柜员先在草稿纸上记一笔"给张三账户 +1000"，然后再慢慢更新正式账本。万一银行突然停电，重新开门后看草稿纸就能把没来得及更新的账目补上 → ⚡**崩溃恢复**
- **undo log（回滚日志）** = 柜员的"撤销记录"。操作前先记下"张三原来余额是 5000"，如果操作到一半你说"不存了"，柜员看撤销记录就能恢复原样 → ⚡**事务回滚**
- **binlog（归档日志）** = 银行的"流水账"。每笔交易完成后记录在案，总行可以根据流水账同步到所有分行（主从复制），出了问题也能根据流水账恢复到任意时间点

**回到技术**："草稿纸"就是 InnoDB 的 redo log，采用 **WAL（Write-Ahead Logging，先写日志再写磁盘）** 机制，事务提交时只需把 redo log 刷盘，数据页可以延迟写入，大幅提升性能。"撤销记录"就是 undo log，记录数据修改前的旧值，回滚时用旧值覆盖，同时也是 MVCC 实现的基础。"流水账"就是 MySQL Server 层的 binlog，记录所有 DDL 和 DML 操作，用于主从复制和基于时间点的数据恢复。

| 对比项 | redo log | undo log | binlog |
|--------|----------|----------|--------|
| 所属层 | ⚡**InnoDB 引擎层** | ⚡**InnoDB 引擎层** | ⚡**MySQL Server 层** |
| 作用 | 崩溃恢复（持久性） | 事务回滚（原子性）+ MVCC | 主从复制 + 数据恢复 |
| 写入时机 | 事务执行过程中持续写入 | 数据修改前写入 | 事务提交时写入 |
| 内容 | 物理日志（哪个数据页改了什么） | 逻辑日志（修改前的旧值） | 逻辑日志（SQL 语句或行变更） |
| 文件大小 | ⚡**固定大小，循环写**（写满了覆盖旧的） | 不固定，事务提交后可清理 | ⚡**追加写，不会覆盖** |

```
事务提交的两阶段提交（保证 redo log 和 binlog 一致）：

① 写 redo log（prepare 状态）
② 写 binlog
③ 提交 redo log（commit 状态）

如果②之后崩溃，重启时发现 redo log 是 prepare 但 binlog 完整 → 提交事务
如果②之前崩溃，redo log 是 prepare 且 binlog 不完整 → 回滚事务
```

**🎤 面试这样答**：
> "MySQL 有三大日志：redo log 是 InnoDB 引擎层的物理日志，用 WAL 机制保证事务持久性，崩溃后可以根据 redo log 恢复未刷盘的数据。undo log 也是 InnoDB 层的，记录数据修改前的旧值，用于事务回滚和 MVCC 的版本链。binlog 是 Server 层的逻辑日志，记录所有数据变更，用于主从复制和数据恢复。事务提交时通过两阶段提交保证 redo log 和 binlog 的一致性。"
