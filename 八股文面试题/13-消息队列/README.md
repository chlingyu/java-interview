...
为什么 Kafka 能做到每秒百万级消息？想象一个快递分拣中心：
- **分区并行** = 开多条传送带同时分拣，而不是所有包裹挤一条线
- **顺序写磁盘** = 包裹按顺序码在货架上，不用到处找空位，码得飞快
- **Page Cache** = 常用的包裹先放在手边的桌子上（内存），不用每次都跑去仓库（磁盘）
- **零拷贝** = 包裹从仓库直接上传送带送走，不用先搬到分拣台再搬到传送带（省了两次搬运）

**回到技术**："多条传送带"就是一个 Topic 分成多个 Partition，多个消费者可以并行消费。"按顺序码"就是消息以 append-only 方式追加写入日志文件，顺序写磁盘的速度接近内存。"手边的桌子"就是操作系统的 Page Cache，写消息先写内存，由 OS 异步刷盘。"直接上传送带"就是 `sendfile()` 系统调用，数据从磁盘直接发到网卡，跳过用户态。
